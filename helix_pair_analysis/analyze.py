...
def main(args):
    ...
    for i in range(len(helix1_1) - args.window + 1):
        ...
        idx11, idx12, idx21, idx22 = 0, 0, 0, 0
    ...
    angle_diff = wrapped_diff(eff_coords_1[1], eff_coords_2[1])
    piston1_diff = eff_coords_1[2] - eff_coords_2[2]
    piston2_diff = eff_coords_1[3] - eff_coords_2[3]
    gearbox1_diff = wrapped_diff(eff_coords_1[4], eff_coords_2[4])
    gearbox2_diff = wrapped_diff(eff_coords_1[5], eff_coords_2[5])
    # '''

    plt.figure()
    plt.plot(resnums, dist_diff, label='Distance difference (Angstroms)')
    plt.plot(resnums, angle_diff, label='Angle difference (Radians)')
    # plt.plot(resnums, piston1_diff, label='Piston 1 difference (Angstroms)')
    plt.plot(resnums, piston2_diff, label='Piston difference (Angstroms)')
    plt.plot(resnums, gearbox1_diff, label='Gearbox 1 difference (Radians)')
    plt.plot(resnums, gearbox2_diff, label='Gearbox 2 difference (Radians)')
